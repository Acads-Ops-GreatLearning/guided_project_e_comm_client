...
export const addCategory = (name, description) => async (dispatch) => {
  try {
    ...
    await axios
      .post(
        BACKEND_URL_ENDPOINT + "/api/v1/categories/",
        { name, description },
        config
      )
      .then((res) => {
...
export const deletCategory = (categoryId) => async (dispatch) => {
  try {
    ...
    await axios
      .delete(BACKEND_URL_ENDPOINT + `/api/v1/categories/${categoryId}`, config)
      .then((res) => {
        if (res.status === 200) {
          console.log(res);
          dispatch({
...
export const listCategories = () => async (dispatch) => {
  try {
    ...
    await axios
      .get(BACKEND_URL_ENDPOINT + "/api/v1/categories/", config)
      .then((res) => {
        if (res.status === 200) {
          console.log(res);
          dispatch({
...
export const fetchCategoryDetails = (categoryId) => async (dispatch) => {
  try {
    ...
    await axios
      .get(BACKEND_URL_ENDPOINT + "/api/v1/categories/" + categoryId, config)
      .then((res) => {
        if (res.status === 200) {
          console.log(res)
...
export const updateCategory =
  (name, description, categoryId) => async (dispatch) => {
    try {
      ...
      await axios
        .put(
          BACKEND_URL_ENDPOINT + "/api/v1/categories/" + categoryId,
