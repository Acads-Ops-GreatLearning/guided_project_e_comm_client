...
export const placeOrderOfUser = (shippingDetails) => async (dispatch) => {
  try {
    ...
    await axios
      .post(BACKEND_URL_ENDPOINT + "/api/v1/orders/", { shippingDetails }, config)
      .then((res) => {
        if (res.status === 200) {
          dispatch({
...
export const getUserOrdersHistory =
  ...
  async (dispatch) => {
    try {
      ...
      await axios
        .get(BACKEND_URL_ENDPOINT + "/api/v1/orders/user/" + userId, config)
        .then((res) => {
          console.log(res);
          if (res.status === 200) {
            dispatch({
              type: USER_ORDER_HISTORY_SUCCESS,
              payload: res.data.orderHistory,
            });
          } else {
            dispatch({
...
export const getAllUsersOrderHistory = () => async (dispatch) => {
  try {
    ...
    await axios
      .get(BACKEND_URL_ENDPOINT + "/api/v1/orders/", config)
      .then((res) => {
...
export const changeUserOrderStatus = (orderId, newStatus) => async (dispatch) => {
  try {
    ...
    await axios
      .put(BACKEND_URL_ENDPOINT + "/api/v1/orders/" + orderId, {newStatus}, config)
      .then((res) => {
        if (res.status === 200) {
          dispatch({
